  ...
  render() {
    ...
    const stations =
      ...
      this.props.stations.map(station => (
        <li
          key={station.id}
          className={`stations-list__item ${station.id === stationNumber + 1 &&
            'active'} ${play &&
        ...
        >
          <div className="stations-list__item-text">{station.title}</div>
        </li>
      ))
    return (
      <Scrollbars className={`stations-list ${active && 'active'}`}>
        <ul>{stations}</ul>
      </Scrollbars>
    )
  }
